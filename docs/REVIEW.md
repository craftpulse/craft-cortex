...
> **Remediation status (2026-05-31):** all four blockers are now **fixed and independently verified** on branch `gate-9-hardening` (`pest` 1023 passed / 0 skipped, PHPStan L8 + ECS clean) — commits `b9868d2` (audit-excerpt + `craft_command` redaction), `3c50ae7` (Skill trashed-handle), `368eff3` (OAuth/`.well-known` `httpEnabled` gate), `2d77cdd` (anonymous OAuth IP throttle), `a139646` (OAuth gc prune + docblock). The findings below are preserved as the point-in-time review. The **majors are the next workstream.**
