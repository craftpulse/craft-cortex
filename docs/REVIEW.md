...
> **Remediation status (updated 2026-06-09):** **all four blockers AND all majors are now fixed and verified** on branch `gate-9-hardening` (`pest` **1077 passed / 0 skipped**, PHPStan L8 + ECS clean). Blockers: `dd65e99` (audit-excerpt + `craft_command` redaction), `8f75c38` (Skill trashed-handle), `fc369ac` (OAuth/`.well-known` `httpEnabled` gate), `a3e89eb` (anonymous OAuth IP throttle), `626b55e` (OAuth gc prune + docblock). Majors: OAuth consent CSRF (`49ba6cc`), Origin fail-closed (`9e94759`), Skill handle format + immutability (`7d08868`, `735d73e`), elevated-session refuse-over-HTTP via a transport-context seam (`5abb762`, `89bf872`), stdio hardening — bounded read / testable framing / fatal-handler / signals+broken-pipe / log-stdout-vector / re-entrancy guard (`13cf6d0`, `8a53a4e`, `26abc8e`, `2484cf9`, `56ce9a8`), `FiberProgressBridge` cancel-drain (`8e766d1`), and the doc majors — EXTENDING/CONFIGURATION/tool-counts + doc-drift comments (`26e26d5`, `8bef504`, `4b2cebe`, `0a414f0`). The findings below are preserved as the point-in-time review. **The minor/nit tail is also closed** (`4524d7e`, `42d1143`, `cd7e0b6`). Two items resolved without code: the Skill `afterSave` "non-transactional" minor was a **false positive** — verified against `craftcms/cms` source, `afterSave()` already runs inside Craft's element-save transaction; and `inputSchemaFor` HTTP field-hiding is **deferred by design** (the execute-level refusal is the security boundary). **Status: full review set remediated on `gate-9-hardening`.**
...
`src/templates/_cp/tokens.twig:7`, `CortexCpAsset.php:20` — *Unverified (nit).* `tokens.twig` and the asset docblock reference 9.2/9.3, but commit 73202a1 swapped Tokens to 9.5; `allowlist.twig:9` already reflects the swap. **Fix:** update the numbers to Tokens = 9.5. **Effort: S.**
