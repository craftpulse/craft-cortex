...
10. **VueAdminTable table-data endpoints return the canonical Craft `AdminTable::paginationLinks()` shape.** `{pagination: {total, per_page, current_page, last_page, next_page_url, prev_page_url, from, to}, data: [row, row, ...]}`. Reference: `vendor/craftcms/cms/src/web/assets/admintable/` Vue component source (verified against the compiled `app.js`). Earlier drafts of this plan claimed `totalPages` — that key does not exist in the upstream contract; use `last_page`. Endpoints: `cortex/allowlist/table-data` (9.2, locked the shape), `cortex/activity/table-data` (9.3), `cortex/tokens/table-data` (9.5). All require `acceptsJson`; all honour `page`, `per_page`, `search`, `sort.0.field`, `sort.0.direction`; Activity additionally honours `filters[kind]`, `filters[toolName]`, `filters[userId]`, `filters[from]`, `filters[to]`. Data-row shape is per-endpoint (the row tuple for Allowlist is `{id, pattern, note, expiresAt, createdBy: {id, label, cpEditUrl} | null, dateCreated, isExpired}` — locked by `SettingsControllerAllowlistTest` architecture invariant). Validation failures return HTTP 400 + `{message}` (Craft framework convention via `asFailure()`); the brief's earlier 422 target was wrong.
